...
COMPORT =5
COMPORT=1 if (COMPORT ==0) else 5   # Check for COMPORT to other than Zero
BAUDRATE= 115200
...
def launch_APK_AUT():
    ...
    waitForObject("Monitoring Activity_Activity.fhr_Text")
    test.compare(findObject(":Monitoring Activity_Activity'").text, 100,"Checking values displayed on the Chart")
    test.compare(waitForObject("Monitoring Activity_Activity.fhr_Text").text, 139)
...
# Check for Device Availability
    try:
        ...
        inst_33220A = rm.open_resource(AGILENT_33220A, read_termination="\r")
        inst_33220A.timeout = 25  
        print(inst_33220A.write(":SOUR1:BURS:INT:PER 0.5 S"))
    #Fail Gracefully
    except IOError:
        print 'cannot Connect to Device: '+ AGILENT_33220A
    except Exception as e:
